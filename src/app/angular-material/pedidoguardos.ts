...
@Component({
    ...
    <mat-card style="width:100%; height:100%">
     ...
     <mat-card-title style="text-align:center">pedidos_x_confirmar</mat-card-title>
      <mat-list style="width: 100%; height:100%">
        <mat-form-field appearance="fill" class="google-search-input">
  <input matInput (keyup)="buscarcliente()"  [(ngModel)]="terminobusqueda">
   <button mat-icon-button matSuffix>
    <mat-icon>search</mat-icon>
  </button>
</mat-form-field>
     
     <virtual-scroller #scroll_productosMostrar [items]="data" style="width: 100%; height:100%">
            <div *ngFor="let _producto of scroll_productosMostrar.viewPortItems" [id]="'p_' + _producto._id" class='row'>
         ...
         <td>{{formatearfecha(_producto.updatedAt)}}</td>
           ...
           <mat-card-actions [class.hiden]="id!==_producto._id">
              <div class="row">
         <div class="col-md-6 col-sm-12 py-0"  style="flex:1;text-align:center;">
           <button mat-flat-button color="primary" full-button pedido_con style="box-shadow:4px 4px 10px rgba(0, 0, 0, 0.2) !important;" [mat-dialog-close]="_producto" >
             Retomar pedido
           </button>
        </div>
        <div class="col-md-6 col-sm-12 py-0"   style="flex:1; text-align:center;" >
         <button mat-flat-button color="primary" full-button pedido_con  style="box-shadow:4px 4px 10px rgba(0, 0, 0, 0.2) !important;" (click)="verdetalles(_producto.productos_pedido)">
           Ver detalles
         </button>
      </div>
      </div>
  ...
  terminobusqueda:string=""
   ...
   datatemporal:any
    ...
    constructor(
        ...
        @Inject(MAT_DIALOG_DATA) public data:Array<datapedido>,private dialog:MatDialog
    ) {
    ...
    buscarcliente(){
      if(this.terminobusqueda==''){
        ...
        this.data=this.datatemporal.filter((item:any)=> item.cliente.nombre.includes(this.terminobusqueda.toUpperCase()) || item.cliente.identificacion.includes(this.terminobusqueda))
      }
